...
{ "HUD_ImGuiDraw", (void **)&clgame.dllFuncs.pfnImGuiDraw },	// Xash3D ext: ImGui support
...
static int GAME_EXPORT pfnHookUserMsg( const char *pszName, pfnUserMsgHook pfn )
{
	...
	if( clgame.msg[i].func && clgame.msg[i].func2 && clgame.msg[i].func3 && clgame.msg[i].func4 )
	{
		// there is no free hook slot
		return 1;
	}
	...
	} else if (!clgame.msg[i].func2) {
		clgame.msg[i].func2 = pfn;
	} else if (!clgame.msg[i].func3) {
		clgame.msg[i].func3 = pfn;
	} else if (!clgame.msg[i].func4) {
		clgame.msg[i].func4 = pfn;
	}
