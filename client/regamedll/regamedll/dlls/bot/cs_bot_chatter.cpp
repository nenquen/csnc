	...
	while (true)
	{
		...
		else if (!Q_stricmp(token, "Place") || !Q_stricmp(token, "Chatter"))
		{
			...
			while (true)
			{
				...
				// g_pFileSystem may be unavailable in some engine configurations.
				// If so, don't crash - just skip the existence check.
				if (g_pFileSystem && !g_pFileSystem->FileExists(filePath))
					continue;
#endif
