...
int CaseInsensitiveHash(const char *string, int iBounds)
{
	...
	// Prevent division by zero when hash table not initialized
	if (iBounds <= 0)
		return 0;
...
void AddEntityHashValue(entvars_t *pev, const char *value, hash_types_e fieldType)
{
	...
	// Guard: if hash table not initialized yet, skip hash value addition
	if (count <= 0)
		return;
...
void RemoveEntityHashValue(entvars_t *pev, const char *value, hash_types_e fieldType)
{
	...
	// Guard: if hash table not initialized, nothing to remove
	if (count <= 0)
		return;
